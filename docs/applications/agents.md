...
Run in a loop until stopped. Useful for:
- Queue processing
...
```
src/agents/my-agent.ts  →  dist/agents/my-agent.js
```

The agent name in `app.json` must match the filename (without extension).
