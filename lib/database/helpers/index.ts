...
export async function getCurrentUser() {
  ...
  return {
    ...
    profilePicture: result.user.data.profilePicture,
...
export async function isFirstTimeSetup(): Promise<boolean> {
  const userManager = new (await import("@/lib/database/managers/user")).default();
...
export async function getUserSubApps(userId: string): Promise<string[]> {
  ...
  const userRecord = await new (await import("@/lib/database/managers/user")).default().readRecord(userId);
...
export function parseSubAppId(fullSubAppId: string): { mainAppId: string; subAppId: string } {
...
export async function getSubApp(fullSubAppId: string): Promise<(SubApp & { mainAppId: string; fullId: string }) | null> {
  try {
...
export async function getUserAuthorizations(userId: string): Promise<{ authorizations: string[] }> {
  const authorityManager = new AuthorityManager();
  const userRecord = await new (await import("@/lib/database/managers/user")).default().readRecord(userId);
...
export async function userHasAuthorization(userId: string, authorization: string): Promise<boolean> {
...
export async function archiveNotification(userIdOrNotificationId: string, timestamp?: number) {
  const notificationManager = new NotificationManager();
  ...
  if (timestamp !== undefined) {
    ...
    for (const key of notificationKeys) {
      ...
      if (recordId) {
        const record = await notificationManager.readRecord(recordId);
        if (record && record.data.userId === userIdOrNotificationId && record.data.timestamp === timestamp) {
    ...
    const notification = await notificationManager.readRecord(userIdOrNotificationId);
...
export async function archiveNotifications(userIdOrNotificationIds: string | string[], timestamps?: number[]) {
...
export async function createUser(
  ...
  const user = await userManager.createRecord(
...
export async function setSystemSetting(key: string, value: string) {
  ...
  if (existing) {
    await settingManager.updateRecord(
...
// Update user
export async function updateUser(userId: string, data: Partial<import("@/lib/database/types/user").default>) {
  const UserManager = (await import("@/lib/database/managers/user")).default;
  const userManager = new UserManager();
  ...
  return await userManager.updateRecord(
    await userManager.getTable(),
    userId,
    { ...user.data, ...data }
  );
...
// Create app
export async function createApp(appId: string, appData: import("@/lib/database/types/app").default) {
  const appManager = new AppManager();
  return await appManager.createRecord(
...
// Update app
export async function updateApp(appId: string, appData: Partial<import("@/lib/database/types/app").default>) {
  const appManager = new AppManager();
  const app = await appManager.readRecord(appId);
  if (!app) {
    throw new Error("App not found");
  }
  return await appManager.updateRecord(
    await appManager.getTable(),
    appId,
    { ...app.data, ...appData }
  );
...
export function compareVersions(v1: import("@/lib/database/types/appVersion").default, v2: import("@/lib/database/types/appVersion").default): number {
...
export async function createAuthorization(authorizationId: string, authorizationData: import("@/lib/database/types/authorization").default) {
  const authorizationManager = new AuthorizationManager();
...
// Create authority
export async function createAuthority(authorityId: string, authorityData: import("@/lib/database/types/authority").default) {
  const authorityManager = new AuthorityManager();
...
// Update authority
export async function updateAuthority(authorityId: string, authorityData: Partial<import("@/lib/database/types/authority").default>) {
  const authorityManager = new AuthorityManager();
  const authority = await authorityManager.readRecord(authorityId);
  if (!authority) {
...
export async function deleteContextualAuthoritiesByApp(appId: string) {
  const ContextualAuthorityManager = (await import("@/lib/database/managers/contextualAuthority")).default;
...
export { formatVersion, isVersionGreaterOrEqual } from "@/lib/database/managers/app";
