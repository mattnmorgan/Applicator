...
- `lib/` - Shared library (internal modules, not directly imported by apps)
- `sdk/` - SDK package (published as `@applicator/sdk` for npm link)
- `app/` - Next.js application pages and API routes
- `public/` - Static assets

### File Structure

- Create new types and interfaces in `types/` subdirectory in separate `ts` files
- Create new classes in separate `ts` files
